	...
	public static function es_sendmail($type = "", $template = 0, $subscribers = array(), $action = "", $post_id = 0, $mailsenttype = "Immediately") {
		...
		if(count($subscribers) > 0) {
			foreach ($subscribers as $subscriber) {
				...
				//subscriber edit profile link input
				$edittext = 'If you would like to change your subscription or your details on our system, 
				please click <a href="' . CreateEditURL( $subscriber["es_email_id"] ) . '">here</a>';

				//End subscriber edit, added by Craig Lambie
				...
				if($wpmail) {  // WP Mail
					...
					if($type == "welcome") {
						if($es_c_usermailoption == "YES") {
							wp_mail($to, $subject, $content_send . $edittext . $unsubtext . $viewstslink, $headers);
						}
					} else {
						if($mailsenttype != "Cron") { 					// Cron mail not sending by this method
							wp_mail($to, $subject, $content_send . $edittext . $unsubtext . $viewstslink, $headers);
					...
					if($type == "welcome") {
						if($es_c_usermailoption == "YES") {
							mail($to ,$subject, $content_send . $edittext . $unsubtext . $viewstslink, $headers);
						}
					} else {
						if($mailsenttype != "Cron") { 					// Cron mail not sending by this method
							mail($to ,$subject, $content_send . $edittext . $unsubtext . $viewstslink, $headers);
